...
Gracefully shut down within 150 seconds by default; pass `timeout` to override the deadline. Deadline expiry, an unreachable local agent, or unclean VM exit raises an error without automatic force-kill. Use `kill()` explicitly for force termination. A failed stop does not confirm that disks can be safely removed.
...
<div className="msb-params">
  <div className="msb-param">
    ...
    <div className="msb-param-desc">Graceful shutdown deadline in seconds. <code>None</code> uses the 150-second default. Expiry raises an error; <code>0</code> provides no grace period and does not force-kill.</div>
  </div>
</div>
