...
Gracefully shut down within an explicit deadline covering request delivery and exit observation. Expiry or unclean exit throws. `0` provides no grace period and does not force-kill.
...
<div className="msb-params">
  <div className="msb-param">
    <div className="msb-param-key"><code>timeoutMs</code><span className="msb-type">number</span></div>
    <div className="msb-param-desc">Graceful shutdown deadline in milliseconds. Expiry throws without automatic force-kill; <code>0</code> provides no grace period.</div>
  </div>
</div>
...
| stopWithTimeout(timeoutMs) | `Promise<void>` | Same as `stop()` with an explicit deadline in milliseconds; `0` provides no grace period |
