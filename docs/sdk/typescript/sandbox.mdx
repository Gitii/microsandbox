...
Gracefully shut down within 150_000 ms. Deadline expiry, an unreachable local agent, or unclean VM exit throws without automatic force-kill. Use `kill()` explicitly for force termination. A failed stop does not confirm that disks can be safely removed.
...
Gracefully shut down within an explicit deadline covering request delivery and exit observation. Expiry or unclean exit throws. `0` is rejected: it leaves no window in which a shutdown could be confirmed — use `kill()` for a forced stop.
...
<div className="msb-params">
  <div className="msb-param">
    <div className="msb-param-key"><code>timeoutMs</code><span className="msb-type">number</span></div>
    <div className="msb-param-desc">Graceful shutdown deadline in milliseconds. Expiry throws without automatic force-kill; <code>0</code> is rejected.</div>
  </div>
</div>
...
| stop() | `Promise<void>` | Gracefully shut down within 150_000 ms; expiry or unclean exit throws without automatic force-kill |
| stopWithTimeout(timeoutMs) | `Promise<void>` | Same as `stop()` with an explicit deadline in milliseconds; `0` is rejected |
