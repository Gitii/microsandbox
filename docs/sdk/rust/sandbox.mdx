...
Gracefully shut down within an explicit deadline covering request delivery and exit observation. Expiry or unclean exit returns an error. `Duration::ZERO` provides no grace period and does not force-kill.
...
| stop_with_timeout(timeout) | `Result<()>` | Same as `stop()` with an explicit deadline; `Duration::ZERO` provides no grace period |
