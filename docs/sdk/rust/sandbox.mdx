...
Gracefully shut down the sandbox within 150 seconds. Deadline expiry, an unreachable local agent, or unclean VM exit returns an error without automatic force-kill. Use `kill()` explicitly for force termination. A failed stop does not confirm that disks can be safely removed.
...
Gracefully shut down within an explicit deadline covering request delivery and exit observation. Expiry or unclean exit returns an error. `Duration::ZERO` is rejected: it leaves no window in which a shutdown could be confirmed — use `kill()` for a forced stop.
...
Stop the sandbox within the default deadline and return its actual successful process exit status. Requires local lifecycle ownership; a reconnected handle returns an error without requesting shutdown. Use [`stop()`](#sb-stop) or [`stop_with_timeout()`](#sb-stop_with_timeout) for reconnected handles. Deadline expiry or unclean exit returns an error.
...
| stop() | `Result<()>` | Gracefully shut down within 150 seconds; expiry or unclean exit returns an error without automatic force-kill |
| stop_with_timeout(timeout) | `Result<()>` | Same as `stop()` with an explicit deadline; `Duration::ZERO` is rejected |
