...
Set the graceful shutdown deadline for [`Stop`](#sb-stop). Default: 150 seconds. Expiry returns an error without automatic force-kill; zero provides no grace period. This is a `StopOption`, not a `SandboxOption`, pass it to [`Stop`](#sb-stop).
