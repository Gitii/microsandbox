...
Gracefully shut down within 150 seconds by default; pass [`WithStopTimeout`](#withstoptimeout) to change the deadline. Deadline expiry, an unreachable local agent, or unclean VM exit returns an error without automatic force-kill. Use `Kill` explicitly for force termination. A failed stop does not confirm that disks can be safely removed.
...
Set the graceful shutdown deadline for [`Stop`](#sb-stop). Default: 150 seconds. Expiry returns an error without automatic force-kill; zero is rejected — use `Kill` for a forced stop. This is a `StopOption`, not a `SandboxOption`, pass it to [`Stop`](#sb-stop).
