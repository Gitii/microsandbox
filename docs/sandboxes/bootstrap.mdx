...
For systemd, graceful shutdown invokes the image's `/usr/bin/systemctl --no-block
poweroff`. This uses the image's own control protocol rather than computing
systemd realtime signals against the agent's musl libc. The image must provide
that executable. Init owns service-stop ordering and filesystem teardown; the
agent does not remount service disks during shutdown or send a fallback signal
that could reboot or re-execute systemd.

The host allows 120 seconds for handoff shutdown, including service stop jobs.
If that deadline expires, forced VMM termination is recorded as a failed,
unclean shutdown. A stopped process alone is not permission to discard or
publish durable disks. Extra disks are not included in root snapshots.
