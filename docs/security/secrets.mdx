...
- **Non-HTTP streams stay opaque.** Once a raw TCP stream is identified as non-HTTP, later bytes are not buffered or rewritten as HTTP headers. SSH ciphertext is forwarded unchanged; configured placeholder-violation checks still apply across writes.
