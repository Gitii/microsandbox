...
```bash
msb stop devbox                # Graceful shutdown (150s deadline)
msb stop --force devbox        # Force kill immediately
msb stop -t 30 devbox          # Fail if clean shutdown is unconfirmed after 30s
msb stop --label app=engine    # stop every sandbox labelled app=engine
```

Graceful shutdown asks the guest to finish shutdown and waits for clean VM
termination. An expired deadline, unreachable agent, or unclean exit fails the
command without automatic force-kill. A failed stop does not confirm that disks
can be safely removed. `--timeout 0` is rejected: no window leaves nothing to
confirm, and `--force` is how a forced stop is requested.

`--force` explicitly kills the VM. Pending writes may be lost, with the same
...
| `-t`, `--timeout` | Graceful shutdown deadline in seconds. Defaults to 150; expiry fails without automatic force-kill. Zero is rejected |
| `-q`, `--quiet` | Suppress progress output |
...
`msb restart` uses the same stop controls as `msb stop`: `--force` kills immediately, and `--timeout` sets the graceful shutdown deadline. If the stop phase fails, the sandbox is not restarted.
...
| `-t`, `--timeout` | Graceful shutdown deadline in seconds. Defaults to 150; expiry fails without restarting. Zero is rejected |
| `-q`, `--quiet` | Suppress progress output |
