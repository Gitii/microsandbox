...
can be safely removed. `--timeout 0` provides no grace period; it is not a force
stop.
...
| `-t`, `--timeout` | Graceful shutdown deadline in seconds. Defaults to 150; expiry fails without automatic force-kill |
| `-q`, `--quiet` | Suppress progress output |
...
| `-t`, `--timeout` | Graceful shutdown deadline in seconds. Defaults to 150; expiry fails without restarting |
| `-q`, `--quiet` | Suppress progress output |
