    ...
    async fn server_read_authenticated<H: Handler + Send, R: Reader>(
        ...
        match msg {
            ...
            msg::CHANNEL_EXTENDED_DATA | msg::CHANNEL_DATA => {
                ...
                let manual = handler.manual_receive_window(channel_num);
                ...
                if let Some(ref mut enc) = self.common.encrypted {
                    if manual {
                        ...
                        // Refuse a peer that exceeds the advertised byte window.
                        channel.sender_window_size = channel
                            .sender_window_size
                            .checked_sub(
                                u32::try_from(data.len()).map_err(|_| Error::Inconsistent)?,
                            )
                            .ok_or(Error::Inconsistent)?;
                ...
                self.flush()?;
                if let Some(ext) = ext {
                    if !manual && let Some(chan) = self.channels.get(&channel_num) {
