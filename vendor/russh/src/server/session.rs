...
impl Session {
    ...
    fn dispatch_msg(&mut self, msg: Msg) -> Result<(), Error> {
        match msg {
            ...
            Msg::Consumed(id, bytes) => {
                if let Some(enc) = self.common.encrypted.as_mut()
                    && let Some(channel) = enc.channels.get_mut(&id)
                {
                    // Defend peer window accounting against caller over-credit.
                    let next = channel
                        .sender_window_size
                        .checked_add(bytes)
                        .filter(|n| bytes > 0 && *n <= self.common.config.window_size)
                        .ok_or(Error::Inconsistent)?;
                    push_packet!(enc.write, {
                        enc.write.push(msg::CHANNEL_WINDOW_ADJUST);
                        channel.recipient_channel.encode(&mut enc.write)?;
                        bytes.encode(&mut enc.write)?;
                    });
                    channel.sender_window_size = next;
                }
            }
