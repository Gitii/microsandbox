...
pub trait Handler: Sized {
    ...
    /// Opt into consumption-driven receive credit for this channel. Data is
    /// delivered only to the handler, never a second channel-reader queue.
    /// Return consumed bytes with `Handle::adjust_receive_window`.
    fn manual_receive_window(&self, _channel: ChannelId) -> bool {
        false
    }
