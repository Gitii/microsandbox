...
use microsandbox_image::{
    GlobalCache, PullOptions, PullProgress, PullProgressSender, PullResult, Reference, Registry,
    ext4, tree,
};
...
use crate::sandbox::{
    FsEntryKind, PullPolicy, RootfsSource, Sandbox, SandboxConfig, SandboxStatus, apply_patches,
    build_upper_tree, remove_dir_if_exists, validate_env, validate_hostname, validate_labels,
    validate_sandbox_name, validate_volume_mounts,
};
    ...
    pub(crate) async fn create_sandbox(
        ...
        if let RootfsSource::Oci(oci) = config.spec.image.clone() {
            ...
            let upper_size_mib = oci
                .upper_size_mib
                .unwrap_or(crate::sandbox::config::DEFAULT_OCI_UPPER_SIZE_MIB);
            ...
            let pull_result = self
                .pull_oci_image(&reference, config.spec.pull_policy, overrides, progress)
                .await?;
            ...
            if let Some(snap_upper) = config.snapshot_upper_source.take() {
                ...
                Self::create_upper_ext4(&upper_path, upper_size_mib, upper_tree).await?;
            }
            ...
            if let Ok(image_ref) = reference.parse::<Reference>() {
                match cache.read_image_metadata_async(&image_ref).await {
                    Ok(Some(metadata)) => {
                        if let Err(e) =
                            crate::image::Image::persist(self, &reference, metadata).await
                        {
                            tracing::warn!(
                                error = %e,
        ...
        let (local_state, returned_config) =
            match self.create_sandbox_inner(config, sandbox_id, mode).await {
                Ok(pair) => pair,
                Err(e) => {
...
mod tests {
    ...
    fn test_validate_rootfs_source_missing_bind_path() {
        ...
        let err =
            LocalBackend::validate_rootfs_source(&RootfsSource::Bind(path.clone())).unwrap_err();
        assert_eq!(
            err.to_string(),
            format!(
    ...
    fn test_validate_rootfs_source_existing_bind_directory() {
        ...
        LocalBackend::validate_rootfs_source(&RootfsSource::Bind(path.clone())).unwrap();
