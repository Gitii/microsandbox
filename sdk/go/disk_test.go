...
func TestOfflineDiskCopy(t *testing.T) {
	...
	created, err := CreateDisk(source, 128*1024*1024)
	if err != nil {
		t.Fatal(err)
	}
	grown, err := GrowDiskCopy(source, destination, 256*1024*1024)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := GrowDiskCopy(source, destination, 512*1024*1024); err == nil {
		t.Fatal("clobbered destination")
	}
	if _, err := GrowDiskCopy(filepath.Join(dir, "missing"), filepath.Join(dir, "blank"), 256*1024*1024); err == nil {
